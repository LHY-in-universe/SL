"""
Qwen2-VL Bottom Model (Vision Front)

职责：仅包含视觉编码器（ViT + patch 合并），输出视觉 token 序列。
输出接口：与官方 Qwen2-VL 一致，返回视觉特征，供后端 Transformer 拼接使用。
"""
...
@ModelRegistry.register("qwen2_vl", "bottom")
class Qwen2VLBottomModel(BaseBottomModel):
    """
    视觉前端：
    ...
    def forward(self, pixel_values: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        """
        Args:
            pixel_values: 图像/视频张量，形状遵循官方 Qwen2-VL 预处理输出
    ...
    def from_pretrained_split(
        ...
        **kwargs,
        ...
        """
        model = cls(config)
        visual_weights = {k.replace("visual.", "", 1): v for k, v in full_state_dict.items() if k.startswith("visual.")}
        missing, unexpected = model.visual.load_state_dict(visual_weights, strict=False)
        if missing:
            print(f"    Warning: Missing keys in Qwen2VLBottomModel: {missing}")
        if unexpected:
            print(f"    Warning: Unexpected keys in Qwen2VLBottomModel: {unexpected}")
        return model
