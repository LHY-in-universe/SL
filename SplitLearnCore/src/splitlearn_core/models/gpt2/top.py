"""
GPT-2 Top Model (Client-side Back Part)

Contains: Last K transformer blocks + Final LN + LM Head
Input: Intermediate hidden states from Trunk
...
@ModelRegistry.register("gpt2", "top")
class GPT2TopModel(BaseTopModel):
    """
    Top (final) part of split GPT-2 model

    Architecture:
        - h: Last K transformer blocks (h[start_layer:n_layer])
        - ln_f: Final layer normalization
        - lm_head: Language modeling head (projects to vocab)

    Args:
        config: GPT2Config object
        start_layer: Starting layer index in original model (default: 10)
    """
    ...
    def get_final_norm(self) -> nn.Module:
        """Return final layer normalization"""
        return self.ln_f
    ...
    def from_pretrained_split(
        ...
        """
        Create Top model from full GPT-2 state dict
        ...
        Returns:
            GPT2TopModel instance with loaded weights
        """
        # Create model
        ...
        top_dict = ParamMapper.filter_and_remap_state_dict(
            ...
            layer_end=config.n_layer,
            remap_layers=True,  # Remap h.10 -> h.0, etc.
        )
