"""
Gemma Top Model (Client-side Back Part)

Contains: Last K transformer blocks + Final norm + LM Head
...
@ModelRegistry.register("gemma", "top")
class GemmaTopModel(BaseTopModel):
    """
    Top (final) part of split Gemma model

    Architecture:
        - layers: Last K transformer blocks (layers[start_layer:])
    ...
    def __init__(self, config: GemmaConfig, start_layer: int = 16):
        super().__init__(config, start_layer)

        # Calculate number of layers for this split
        num_layers_in_split = self.end_layer - start_layer

        # Transformer blocks (remapped to start from 0)
        self.layers = nn.ModuleList(
            [
                GemmaDecoderLayer(config, layer_idx=i)
                for i in range(num_layers_in_split)
            ]
        )
    ...
    def get_final_norm(self) -> nn.Module:
        """Return final normalization layer (RMSNorm)"""
        return self.norm
    ...
    def from_pretrained_split(
        ...
        """
        Create Top model from full Gemma state dict
        ...
        Returns:
            GemmaTopModel instance with loaded weights
        """
        # Create model
        model = cls(config, start_layer=start_layer)

        # Get total number of layers
        num_layers = getattr(config, 'num_hidden_layers', 18)
        ...
        # Remove 'model.' prefix from keys, but keep 'lm_head' as is
        top_dict_clean = {}
        for key, value in top_dict.items():
            if key.startswith('model.'):
                clean_key = key.replace('model.', '', 1)
                top_dict_clean[clean_key] = value
            else:
                # lm_head is not prefixed with 'model.'
                top_dict_clean[key] = value

        # Load weights
        missing_keys, unexpected_keys = model.load_state_dict(top_dict_clean, strict=False)
        if missing_keys:
            print(f"    Warning: Missing keys in GemmaTopModel: {missing_keys}")
        if unexpected_keys:
            print(f"    Warning: Unexpected keys in GemmaTopModel: {unexpected_keys}")
