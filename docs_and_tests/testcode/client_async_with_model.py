...
# 测试配置
SERVER_ADDRESS = "localhost:50061"  # 异步服务器端口
TIMEOUT = 30.0
...
async def test_connection(client):
    ...
    if await client.connect():
        ...
        print("   ❌ 连接失败！")
        print(f"\n💡 请确保服务器正在运行:")
        print(f"   python testcode/server_async_with_model.py")
        return False
    ...
    for i in range(num_requests):
        ...
        try:
            ...
            if output.shape == test_input.shape:
                ...
                print(f"   ⚠️  输出形状不正确: {output.shape} (期望: {test_input.shape})")
...
async def async_main():
    """异步主函数"""
    print("\n" + "=" * 70)
    print("💻 异步 gRPC 客户端（使用模型文件）")
    print("=" * 70)
    print(f"\n📡 服务器地址: {SERVER_ADDRESS}")
    print(f"⏱️  超时时间: {TIMEOUT} 秒")
    print(f"💡 服务器使用: testcode/gpt2_trunk_full.pt")
