"""
Qwen2 Top Model (Client-side Back Part)
...
@ModelRegistry.register("qwen2", "top")
class Qwen2TopModel(BaseTopModel):
    """
    Top (final) part of split Qwen2 model
    ...
    def __init__(self, config: Qwen2Config, start_layer: int = 24):
    ...
    def from_pretrained_split(
        ...
        """
        Create Top model from full Qwen2 state dict
        ...
        Returns:
            Qwen2TopModel instance with loaded weights
        """
        # Create model
        ...
        if missing_keys:
            print(f"    Warning: Missing keys in Qwen2TopModel: {missing_keys}")
        if unexpected_keys:
            print(f"    Warning: Unexpected keys in Qwen2TopModel: {unexpected_keys}")
