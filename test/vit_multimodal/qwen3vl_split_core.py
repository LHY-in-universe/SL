#!/usr/bin/env python3
"""
Qwen3-VL-2B 拆分示例（视觉塔 → 文本 Transformer）

说明：
- 使用 SplitLearnCore 的 quickstart，根据 split_points 切分 Qwen3-VL：
  - bottom：视觉塔（ViT+Merger），输出视觉特征序列
  - trunk/top：文本 Transformer 层与 lm_head
- 默认 split_points=[0,14]，表示视觉塔在 bottom，文本层对半分到 trunk/top。
"""
import argparse
import torch
...
def parse_args():
    p = argparse.ArgumentParser(description="qwen3-vl 拆分示例")
    p.add_argument("--model", default="Qwen/Qwen3-VL-2B-Instruct", help="模型名称或本地路径")
...
def main():
    ...
    processor = Qwen2VLProcessor.from_pretrained(args.model, cache_dir="./models")
